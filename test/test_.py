...
#test if there is a correct response (error 500) for using faulty data using questionnaire_upd endpoint
def test_upd0_faulty():
	runner = CliRunner()
	with runner.isolated_filesystem():
		with open("file.txt", "w") as f:
			f.write(json.dumps(que0_faulty))
		result = runner.invoke(cli, ["questionnaire-upd", "--source", "file.txt","--format","json"])
		status_code, output = result.output.split('\n')[:2]
	assert int(status_code) == 500
	assert result.exit_code == 0

#test if we can upload a new questionnaire (QQ000)
def test_upd0():
	runner = CliRunner()
	with runner.isolated_filesystem():
		with open("file.txt", "w") as f:
			f.write(json.dumps(que0))
		result = runner.invoke(cli, ["questionnaire-upd", "--source", "file.txt","--format","json"])
		status_code, output = result.output.split('\n')[:2]
	assert int(status_code) == 200
	assert result.exit_code == 0

#test if we can upload a new questionnaire (QQ001)
def test_upd1():
	runner = CliRunner()
	with runner.isolated_filesystem():
		with open("file.txt", "w") as f:
...
def test_question():  
	runner = CliRunner()
	result = runner.invoke(cli,["question","--questionnaire_id","QQ001","--question_id","Q01","--format","json"])
	status_code, output = result.output.split('\n')[:2]
	output = json.loads(output)
	assert int(status_code) == 200
	assert result.exit_code == 0
	assert output[0]['questionnaireID'] == 'QQ001'
	assert output[0]['qID'] == 'Q01'
	assert output[0]['qtext'] == 'Ποια είναι η ηλικία σας;'
	assert output[0]['required'] == 'TRUE'
	assert output[0]['type'] == 'profile'
	assert output[0]['options'][0]['optID'] == 'Q01A1'
	assert output[0]['options'][1]['optID'] == 'Q01A2'
	assert output[0]['options'][2]['optID'] == 'Q01A3'
	assert output[0]['options'][3]['optID'] == 'Q01A4'

#test getsessionanswers endpoint using questionnaire QQ001 for sessions 0001 and 0002 
#for the first one we expect 402 and the second one is expected to return status code 200
def test_getsessionanswers():  
  runner = CliRunner()
  result = runner.invoke(cli,["getsessionanswers","--questionnaire_id","QQ001","--session_id","0001","--format","json"])
  result1 = runner.invoke(cli,["getsessionanswers","--questionnaire_id","QQ001","--session_id","0002","--format","json"])	
  status_code, output = result.output.split('\n')[:2]
  status_code1, output1 = result1.output.split('\n')[:2]  
  output = json.loads(output)
  output1 = json.loads(output1)
  assert int(status_code) == 200
  assert int(status_code1) == 402  
  assert result.exit_code == 0
  assert output[0]['questionnaireID'] == 'QQ001'
  assert output[0]['session'] == '0001'
  assert output1['status'] == "No data"

#test getquestionanswers endpoint using questionnaire QQ001 and question Q01
def test_getquestionanswers():
  runner = CliRunner()
  result = runner.invoke(cli,["getquestionanswers","--questionnaire_id","QQ001","--question_id","Q01","--format","json"])
  status_code, output = result.output.split('\n')[:2]
  output = json.loads(output)
  assert int(status_code) == 200
  assert result.exit_code == 0
  assert output[0]['questionnaireID'] == 'QQ001'
  assert output[0]['questionID'] == 'Q01'
  assert output[0]['answers'][0]['session'] == '0001'
  assert output[0]['answers'][0]['ans'] == 'Q01A1'
...
#questionnaire QQ000 dummy data
que0 = {
...
#questionnaire QQ001 dummy data
que1 = {
  "questionnaireID": "QQ001",
  "questionnaireTitle": "My first research questionnaire",
  "keywords": ["football"],
